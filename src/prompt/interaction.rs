...
pub trait SkippablePrompt<T> {
    ...
    fn interact(&mut self) -> io::Result<Option<T>> {
        self.interact_on(&mut Term::stderr())
    }
    /// Starts the interaction with the user via the given terminal.
    fn interact_on(&mut self, term: &mut Term) -> io::Result<Option<T>> {
        if !term.is_term() {
    ...
    fn interact_on_prepared(&mut self, term: &mut Term) -> io::Result<Option<T>> {
        ...
        loop {
            ...
            match state {
                State::Submit(result) => return Ok(Some(result)),
                State::Cancel => return Err(io::ErrorKind::Interrupted.into()),
                State::Skipped => return Ok(None),
                _ => {}
            }
